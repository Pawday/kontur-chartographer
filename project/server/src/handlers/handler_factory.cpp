...
Poco::Net::HTTPRequestHandler* HandlerFactory::createRequestHandler(const HTTPServerRequest& request)
{
    ...
    if (reqMethod == "POST")
    {
        ...
        if (reqParams.size() >= 2)
        {
            ...
            {
                if (reqParams[i].first == "width") widthStrParam = reqParams[i].second;
                if (reqParams[i].first == "height") heightStrParam = reqParams[i].second;
                if (reqParams[i].first == "x") xStrParam = reqParams[i].second;
                if (reqParams[i].first == "y") yStrParam = reqParams[i].second;
            }
